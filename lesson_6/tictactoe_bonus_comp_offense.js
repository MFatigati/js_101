...
function detectImmediateVictory(board) {
  for (let line = 0; line < WINNING_LINES.length; line ++) {
    ...
    if (victoryCounter === 2 && occupiedByHuman === 0) {
      return line;
    }
}}

function claimImmediateVictory(board, availableWinningLine) { // this is somehow overridding my choice, if I try to defend against the computer
 ...
 */
    ...
    if (typeof detectImmediateVictory(board) === "number") {
      claimImmediateVictory(board, detectImmediateVictory(board))
    } else if (typeof detectImmediateThreat(board) === "number") {
